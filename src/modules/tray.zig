...
// ---------------------------------------------------------------------------
// Menu model (independent of D-Bus, unit-tested)
// ---------------------------------------------------------------------------

pub const Menu = struct {
    arena: std.heap.ArenaAllocator,
    /// Index = dbusmenu item ID; 0 is the root.
    ...
    pub fn properties(self: *Menu, id: i32) *glib.Variant {
        const n = self.node(id) orelse return emptyDict();
    ...
    /// `(ia{sv}av)`.
    pub fn layout(self: *Menu, id: i32, depth: i32) *glib.Variant {
        var children: std.ArrayList(*glib.Variant) = .empty;
        defer children.deinit(std.heap.smp_allocator);
        if (depth != 0) {
            if (self.node(id)) |n| {
                for (n.children) |child| {
                    children.append(std.heap.smp_allocator, glib.Variant.newVariant(self.layout(child, depth - 1))) catch break;
                }
            }
        }
...
pub const Tray = struct {
    ...
    fn methodCall(
        ...
        if (std.mem.eql(u8, method, "GetLayout")) {
            ...
            var out = [_]*glib.Variant{ glib.Variant.newUint32(self.menu.revision), self.menu.layout(parent, depth) };
            ...
            for (0..ids.nChildren()) |i| {
                ...
                var pair = [_]*glib.Variant{ glib.Variant.newInt32(id), self.menu.properties(id) };
            ...
            const props = glib.Variant.refSink(self.menu.properties(childInt(params, 0)));
            defer props.unref();
...
pub fn check(gpa: std.mem.Allocator, ctx: oriel.CheckContext) !oriel.Check {
    ...
    const layout = try printVariant(gpa, menu.layout(0, -1));
...
test "menu layout" {
    ...
    const text = try printVariant(std.testing.allocator, menu.layout(0, -1));
    ...
    const shallow = try printVariant(std.testing.allocator, menu.layout(4, 0));
