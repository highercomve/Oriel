...
//!     };
//!
//! The frontend calls `ziguri.invoke("greet", { name: "Ada" })`. Arguments are
...
pub fn typescript(comptime Commands: type, comptime Events: type) []const u8 {
    comptime {
        ...
        return 
        \\// Generated by ziguri from the Zig API. Do not edit.
        \\
        ...
        ++ events ++
            ...
            \\  return window.ziguri.invoke(cmd, args[0] ?? null) as Promise<Commands[K]["result"]>;
            \\}
            \\
            ...
            \\  return window.ziguri.listen(event, callback as (payload: unknown) => void);
            \\}
            \\
        ;
    }
